#!/usr/bin/env python3
"""End-to-end processing of parametric tank OpenFOAM cases with pressure field.

Pipeline steps:
- Read U (vector) and nut (scalar) and p (scalar) fields at time=1800 for each CubeXX case.
...
def process_case(
    ...
    p = read_scalar_field(time_dir / "p", expected=None)

    n_cells = max(base_cells, len(U), len(nut), len(p))
    ...
    p_grid = np.where(mask_geom, p_grid, np.nan)
    ...
    if case_name.lower() == "cube03":
        ...
        p_vis = np.where(mask, p_grid, np.nan)
        ...
        write_summary_png(p_vis, "summary_p.png", "viridis", "p", overlay_mask=mask)
...
def main():
    parser = argparse.ArgumentParser(description="Process OpenFOAM tank cubes to HDF5 with pressure.")
    parser.add_argument("--base", type=Path, default=Path(__file__).parent, help="Base directory containing CubeXX")
    parser.add_argument(
        "--out",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "data" / "processed_data_with_pressure.h5",
        help="Output HDF5 file",
    )
    parser.add_argument(
        "--plots",
        type=Path,
        default=Path(__file__).parent / "plots" / "case03_y_slices_with_pressure",
